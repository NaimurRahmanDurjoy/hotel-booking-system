...
def edge(id, value, source, target, exitX=0.5, exitY=1, entryX=0.5, entryY=0, extra_pts="", parent="1"):
    ...
    # Forced orthogonal style
    style = f"endArrow=classic;html=1;strokeColor=#000000;strokeWidth=1;rounded=0;labelBackgroundColor=#ffffff;edgeStyle=orthogonalEdgeStyle;exitX={exitX};exitY={exitY};entryX={entryX};entryY={entryY};"
    return f'''        <mxCell id="{id}" value="{safe_value}" style="{style}" edge="1" parent="{parent}" source="{source}" target="{target}">
...
def perfect_login_swimlane():
    c = node("pool", "Login & Reset Process", s_lane, 50, 50, 600, 1000)
    c += node("lane_u", "User", s_sublane, 0, 0, 300, 1000, "pool")
    c += node("lane_s", "System", s_sublane, 300, 0, 300, 1000, "pool")
    
    y = 40
    c += node("start", "", s_start, 135, y, 30, 30, "lane_u")
    y += 80
    c += node("u1", "Clicks login", s_action, 50, y, 200, 60, "lane_u")
    y += 80
    c += node("u2", "Mobile/email & password", s_action, 50, y, 200, 60, "lane_u")
    
    # System decision at same Y as u2
    c += node("d1", "Login Successful?", s_decision, 100, y - 10, 100, 80, "lane_s")
    
    y += 120
    c += node("u3", "Book Room/Car", s_action, 50, y, 200, 60, "lane_u")
    c += node("s1", "Redirected to reset", s_action, 50, y, 200, 60, "lane_s")
    
    y += 100
    c += node("u4", "Click forgot password", s_action, 50, y, 200, 60, "lane_u")
    
    y += 100
    c += node("u5", "Provides email", s_action, 50, y, 200, 60, "lane_u")
    
    y += 100
    c += node("u6", "Enter new password", s_action, 50, y, 200, 60, "lane_u")
    
    y += 100
    c += node("u7", "Confirm password", s_action, 50, y, 200, 60, "lane_u")
    
    y += 100
    c += node("s2", "Password reset successfully", s_action, 50, y, 200, 60, "lane_s")
    ...
    c += edge("e4", "Yes", "d1", "u3", 0, 1, 1, 0.5, "<mxPoint x='350' y='360'/>", parent="pool")
    ...
    c += edge("e9", "", "u6", "u7", parent="lane_u")
    c += edge("e10", "", "u7", "s2", 1, 0.5, 0, 0.5, parent="pool")
    c += edge("e11", "", "s2", "end", parent="lane_s")
    
    return xml_template.replace("{name}", "Login Swimlane").replace("{content}", c)

with open("Grand_Azure_Login_Swimlane.drawio", "w") as f:
    f.write(perfect_login_swimlane())
