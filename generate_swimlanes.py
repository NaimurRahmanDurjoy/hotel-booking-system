...
def reg_swimlane():
    ...
    c += node("u1", "clicks on purchase", s_action, 50, 100, 200, 60, "lane_u")
    c += node("u2", "fills out registration form", s_action, 50, 300, 200, 60, "lane_u")
    c += node("u3", "provides mobile & NID", s_action, 50, 400, 200, 60, "lane_u")
    c += node("u4", "Successfully registered", s_action, 50, 550, 200, 60, "lane_u")
    
    # System Lane Nodes
    c += node("s1", "Redirect to login/registration", s_action, 50, 100, 200, 60, "lane_s")
    c += node("d1", "register?", s_decision, 100, 250, 100, 80, "lane_s")
    c += node("s2", "login page", s_action, 180, 350, 100, 60, "lane_s")
    c += node("end", "", s_end, 215, 450, 30, 30, "lane_s")
    c += node("s3", "Submit form", s_action, 50, 550, 200, 60, "lane_s")
    
    # Edges (Need global coordinates or relative to lanes)
    c += edge("e1", "", "start", "u1", parent="lane_u")
    c += edge("e2", "", "u1", "s1", 1, 0.5, 0, 0.5, parent="pool")
    c += edge("e3", "", "s1", "d1", parent="lane_s")
    c += edge("e4", "No", "d1", "s2", 1, 0.5, 0.5, 0, "<mxPoint x='230' y='290'/>", parent="lane_s")
    c += edge("e5", "Yes", "d1", "u2", 0, 0.5, 1, 0.5, parent="pool")
    c += edge("e6", "", "u2", "u3", parent="lane_u")
    c += edge("e7", "", "u3", "s3", 0.5, 1, 0, 0.5, "<mxPoint x='150' y='500'/><mxPoint x='350' y='500'/>", parent="pool")
...
# 2. Login Swimlane
def login_swimlane():
    c = node("pool", "Login & Reset Process", s_lane, 50, 50, 600, 800)
    c += node("lane_u", "User", s_sublane, 0, 0, 300, 800, "pool")
    c += node("lane_s", "System", s_sublane, 300, 0, 300, 800, "pool")
    
    c += node("start", "", s_start, 135, 40, 30, 30, "lane_u")
    c += node("u1", "Clicks login", s_action, 50, 100, 200, 60, "lane_u")
    c += node("u2", "Mobile/email & password", s_action, 50, 180, 200, 60, "lane_u")
    c += node("d1", "Login?", s_decision, 100, 170, 100, 80, "lane_s")
    c += node("u3", "Book Room/Car", s_action, 50, 300, 200, 60, "lane_u")
    c += node("s1", "Redirect to reset pass", s_action, 50, 300, 200, 60, "lane_s")
    c += node("s2", "Click forgot password", s_action, 180, 420, 100, 60, "lane_s") # Typo in ref but kept context
    c += node("u4", "Provides email", s_action, 50, 420, 150, 60, "lane_u")
    c += node("u5", "Enter new password", s_action, 50, 520, 150, 60, "lane_u")
    c += node("u6", "Confirm password", s_action, 160, 520, 100, 60, "lane_u")
    c += node("s3", "Password reset successfully", s_action, 50, 520, 200, 60, "lane_s")
    c += node("end", "", s_end, 135, 650, 30, 30, "lane_s")

    c += edge("e1", "", "start", "u1", parent="lane_u")
    c += edge("e2", "", "u1", "u2", parent="lane_u")
    c += edge("e3", "", "u2", "d1", 1, 0.5, 0, 0.5, parent="pool")
    c += edge("e4", "Yes", "d1", "u3", 0, 0.5, 1, 0.5, parent="pool")
...
with open("Grand_Azure_Registration_Swimlane.drawio", "w") as f:
    f.write(reg_swimlane())
with open("Grand_Azure_Login_Swimlane.drawio", "w") as f:
    f.write(login_swimlane())
# Booking swimlane can be added similarly
